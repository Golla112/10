...
export const LIST_MARKET_FILTERS = [
  ...
  { id: 'totals', label: 'U/O 2.5', marketKeys: ['totals'] },
] as const;

export type ListMarketFilterId = (typeof LIST_MARKET_FILTERS)[number]['id'];
...
export function marketKeysForFilter(filterId: string): string[] {
  const all = [...SOCCER_MARKET_FILTERS, ...TENNIS_MARKET_FILTERS, ...BASKETBALL_MARKET_FILTERS];
  const g = all.find((x) => x.id === filterId);
  if (g) return [...g.marketKeys];
  const legacy = BETCODE_MARKET_GROUPS.find((x) => x.id === filterId);
  return legacy ? [...legacy.marketKeys] : [filterId];
}

export type SportCategory = 'soccer' | 'tennis' | 'basketball';

export const SOCCER_MARKET_FILTERS = BETCODE_MARKET_GROUPS;

export const TENNIS_MARKET_FILTERS = [
  { id: 'tennis_h2h', label: 'Vincente', marketKeys: ['tennis_h2h'] },
  { id: 'tennis_set1', label: '1° Set', marketKeys: ['tennis_set1'] },
  { id: 'set_betting', label: 'Set Betting', marketKeys: ['set_betting'] },
  { id: 'totals_15', label: 'U/O 1.5', marketKeys: ['totals_15'] },
  { id: 'totals', label: 'U/O 2.5', marketKeys: ['totals', 'totals_25'] },
  { id: 'totals_35', label: 'U/O 3.5', marketKeys: ['totals_35'] },
] as const;

export const BASKETBALL_MARKET_FILTERS = [
  { id: 'h2h', label: 'Vincente', marketKeys: ['h2h'] },
  { id: 'spread', label: 'Handicap', marketKeys: ['spread'] },
  { id: 'totals_15', label: 'U/O 1.5', marketKeys: ['totals_15'] },
  { id: 'totals', label: 'U/O 2.5', marketKeys: ['totals', 'totals_25'] },
  { id: 'totals_35', label: 'U/O 3.5', marketKeys: ['totals_35'] },
] as const;

export function marketFiltersForSport(sport: string) {
  if (sport === 'tennis') return TENNIS_MARKET_FILTERS;
  if (sport === 'basketball') return BASKETBALL_MARKET_FILTERS;
  return SOCCER_MARKET_FILTERS;
}

export type MarketFilterDef = {
  id: string;
  label: string;
